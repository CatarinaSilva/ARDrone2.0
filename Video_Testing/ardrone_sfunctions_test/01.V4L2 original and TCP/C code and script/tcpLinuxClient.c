...
#ifndef MATLAB_MEX_FILE
	double nbytes = 0;
	int s = connect_TCPclient("192.168.1.2", clientPortWorkVector[0]);
	
	if(s!=-1){
		nbytes = TCP_send(s,(char *) imageVectorInput, 1843200);
		close(s);
	} 
...
double imageSend2(unsigned char* imageVectorInput, int * clientPortWorkVector)
{
	...
	if(s!=-1){
		nbytes = TCP_send(s,(char *) imageVectorInput, 153600);
		close(s);
	} 
